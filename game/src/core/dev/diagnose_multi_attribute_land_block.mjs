...
// Geometry + attribute-map rotation are one transform.
{
...
// Internal terrain transition is card-owned: GL0 <-> GL2 inside one block is legal.
{
...
// External edge still uses the touching cell's own semantic.
{
    const state = createState();
    const grid = new GridEngine(state);
...
// Materialization keeps one block identity but distinct cell terrain.
// Same-placement merge propagation must not absorb an incompatible terrain cell.
{
    const state = createState();
    placeExisting(state, 2, 0, PLAINS, "existing");
    const grid = new GridEngine(state, {
...
// Trial ownership remains block-level even when terrain interaction is cell-level.
{
    const state = createState();
...
// Hand/Reserve persistence keeps the rotated attribute map and does not invent
// a card-level terrainId for a multi-attribute card.
{
    const state = createState();
