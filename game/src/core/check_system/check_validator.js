...
export function validateDefinition(key, def) {
    ...
    for (const outcome of def.outcomes) {
        ...
        if (outcome.min !== undefined && outcome.min !== null && outcome.max !== undefined && outcome.max !== null && outcome.min > outcome.max) {
    ...
    const intervals = def.outcomes.map((outcome) => ({
        id: outcome.id,
        lower: outcome.min !== undefined && outcome.min !== null ? outcome.min : -Infinity,
        upper: outcome.max !== undefined && outcome.max !== null ? outcome.max : Infinity
    }));
...
export const CheckValidator = {
    validateDefinitions: validateCheckDefinitions,
    validateDefinition: (def) => validateDefinition(def.id || "anonymous", def)
};
