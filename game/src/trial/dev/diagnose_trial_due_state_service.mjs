...
// Due: exactly one semantic request containing only Trial index.
...
// Repeated/overdue boundaries do not duplicate or replace the same request.
gameFactHub.emit(GAME_FACT_TYPES.VERSE_COMMITTED, { completedTurn: 15, nextTurn: 16 });
assert.deepEqual(dueStateService.getPendingRequest(), { trialIndex: 1 });

// Presentation acknowledges only the matching pending Trial.
...
// Overdue Verse boundaries cannot recreate an acknowledged request while the
// timing authority still points at the same Trial.
gameFactHub.emit(GAME_FACT_TYPES.VERSE_COMMITTED, { completedTurn: 16, nextTurn: 17 });
assert.equal(dueStateService.getPendingRequest(), null);

// Settlement advances timing authority; Trial 2 can become pending later.
