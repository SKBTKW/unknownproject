...
// Outside the five-Verse semantic warning window: no timing-driven change.
...
// Five Verses from Trial: semantic TENSE only; no exact countdown is exposed.
...
// Settlement advances exact timing; Warning reset is owned by the existing
// settlement bridge in production, represented directly here to isolate this bridge.
timingAuthority.markTrialSettled(1);
warningStateService.resetForNextTrial({ source: "TEST_SETTLEMENT", verse: 15 });
assert.equal(warningStateService.getState(), WARNING_STATES.CALM);

// The same bridge now evaluates distance against Trial 2.
