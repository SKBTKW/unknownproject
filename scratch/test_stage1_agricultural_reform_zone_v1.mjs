...
function plainsCell(r, c, groupId) {
    return {
        ...emptyCell(r, c),
        placed: true,
        merged: true,
...
function forestCell(r, c, groupId) {
    return {
        ...emptyCell(r, c),
        placed: true,
        merged: true,
...
function makeState() {
    ...
    for (const pt of plains) grid[pt.r][pt.c] = plainsCell(pt.r, pt.c, "zone_plains");
    for (const pt of forest) grid[pt.r][pt.c] = forestCell(pt.r, pt.c, "zone_forest");
    grid[2][2] = {
        ...emptyCell(2,2),
        placed: true,
        isHQ: true,
...
assert.equal(state.permanentPlainsFoodBonus, undefined,
    "Agricultural Reform must not write the legacy global plains bonus");
...
assert.equal(after.grossFood - baseline.grossFood, 4,
    "Agricultural Reform adds +1 food per member of the 4-cell Zone exactly once");
...
assert.equal(duplicateTargets.length, 0,
    "already-converted Zone is no longer a legal Agricultural Reform target");
