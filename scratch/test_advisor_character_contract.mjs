...
assert.ok(characters.includes(DEFAULT_ADVISOR_CHARACTER), 'default advisor must be one of the registered character definitions');
...
for (const character of characters) {
    ...
    assert.equal(Object.isFrozen(character.adviceDialogue), true);
...
for (const character of characters) {
    ...
    if (character.reactions?.[ADVISOR_SCENES.TRIAL_INTERCEPTION_CONFIRMED]) {
        ...
        assert.ok(received[0].priority >= 70);
    } else {
