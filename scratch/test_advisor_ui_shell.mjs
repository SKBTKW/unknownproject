...
check(openTimer?.delay === 0 && ADVISOR_UI_TIMING.HOVER_OPEN_MS === 0, "右端hoverで待ち時間なく展開する");
openTimer.callback();
...
check(dock.viewState === ADVISOR_VIEW_STATES.EXPANDED && dock.expandedReason === ADVISOR_EXPANDED_REASONS.CLICK, "hover一時展開中の顔clickで固定展開へ昇格する");
dock.collapse();
dock.scheduleHoverExpand();
timers.get(dock.hoverOpenTimer).callback();
...
check(dockSource.includes("advisor-depth-menu") && !dockSource.includes("advisor-report-bubble-depth"), "深度切替を吹き出し外の報告menuへ置く");
...
check(css.includes(".advisor-dock.is-expanded .advisor-portrait-viewport") && css.includes("background: transparent"), "展開portraitの暗い枠背景を撤去する");
check(css.includes("border: 3px solid #b71218") && css.includes("rgba(255, 255, 255, 0.97)"), "報告を白地と赤枠の吹き出しにする");
