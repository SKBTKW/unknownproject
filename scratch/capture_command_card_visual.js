const { chromium } = require('playwright');

(async () => {
    const browser = await chromium.launch();
    const page = await browser.newPage({ viewport: { width: 1280, height: 800 } });

    await page.goto('http://localhost:8080/index_v2.html', { waitUntil: 'domcontentloaded' });
    await page.waitForSelector('#gridBoard .cell', { state: 'attached', timeout: 10000 });

    // Inject command cards into handOffering
    await page.evaluate(() => {
        const drawSys = new Step1DrawSystem(window.state);
        const master = drawSys.getLandCardMaster();
        const milCard = master.find(c => c.id === "CMD_MILITARY_FOCUS");
        const landCard = master.find(c => c.id === "CMD_LAND_FOCUS");

        if (milCard) window.state.handOffering[0] = milCard;
        if (landCard) window.state.handOffering[1] = landCard;
        render();
    });

    const screenshotPath = 'C:/Users/mam07/.gemini/antigravity/brain/af6d4803-5dd7-4a55-87d7-472ecdecfde4/command_card_desc_real_preview.png';
    const cardRow = await page.$('#cardRow');
    if (cardRow) {
        await cardRow.screenshot({ path: screenshotPath });
        console.log(`Command card screenshot saved to ${screenshotPath}`);
    }

    await browser.close();
})();
