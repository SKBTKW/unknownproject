...
async function main() {
    ...
    const stage1AgriculturalReformZoneV1Ok = await runCommand("node", ["scratch/test_stage1_agricultural_reform_zone_v1.mjs"]);
    if (!stage1AgriculturalReformZoneV1Ok) {
        console.error("\n❌ [PIPELINE BLOCKED] Layer 3 (Stage1 Agricultural Reform Zone v1) で不合格が検出されました。");
        process.exit(1);
    }
    ...
    const trialCoreDefenseWriteThroughOk = await runCommand("node", ["scratch/test_trial_core_defense_write_through.mjs"]);
    if (!trialCoreDefenseWriteThroughOk) {
        console.error("\n❌ [PIPELINE BLOCKED] Layer 4 (Trial Core Defense Write-through) で不合格が検出されました。");
        process.exit(1);
    }
