...
async function main() {
    console.log("\n============================================================");
    console.log("🛡️  AG_ToA マスター完全統合検問パイプライン (全自動 5段階検査)");
    console.log("============================================================\n");

    const startTime = Date.now();

    // 📜 Step 1: 仕様書 ＆ 直書き日本語検問
    console.log("🔍 [STEP 1/5] 仕様書 ＆ コード数値突合検問 (Python)...");
    ...
    console.log("\n🧪 [STEP 2/5] ゲームロジック ＆ 経済サイクル全自動検問 (Node.js)...");
    ...
    if (!trialPreviewOk) {
        console.error("\n❌ [PIPELINE BLOCKED] Step 2 (Trial Phase 2迎撃比較検問) で不合格が検出されました。");
        process.exit(1);
    }
    const trialDevHarnessOk = await runCommand("node", ["scratch/test_trial_phase25_dev_harness.mjs"]);
    if (!trialDevHarnessOk) {
        console.error("\n❌ [PIPELINE BLOCKED] Step 2 (Trial Phase 2.5開発プレビュー検問) で不合格が検出されました。");
        process.exit(1);
    }
    ...
    // 🌾 Step 5: 食料総産出・維持費・不足補填検問
    console.log("\n🌾 [STEP 5/5] 食料決済 ＆ 不足補填検問 (Node.js)...");
    const step5Ok = await runCommand("node", ["scratch/test_maintenance_fallback_system.mjs"]);
    if (!step5Ok) {
        console.error("\n❌ [PIPELINE BLOCKED] Step 5 (食料決済) で不合格が検出されました。");
        process.exit(1);
    }
    ...
    console.log("   ✅ Trial Phase 2迎撃比較検証コマンド PASS");
    console.log("   ✅ Trial Phase 2.5開発プレビュー検証コマンド PASS");
