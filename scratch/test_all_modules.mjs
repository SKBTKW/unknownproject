...
saveEngine.state.defenseSystem.increaseMaxCapacity(10);
saveEngine.state.defenseSystem.setCurrentDefense(0);
const gained = saveEngine.state.gainDefense(10, 'テスト獲得');
assert(gained === 13 && saveEngine.state.getCurrentDefense() === 13, '次ターンの警戒適用中に現在防衛力10回復で +3 ボーナスが乗って 13 回復すること');
...
condEngine.state.defenseSystem.increaseMaxCapacity(15);
condEngine.state.checkConditionalBuffs();
...
assert(pairLinkEngine.state.gameLogs.some(log => log.includes('連携')), '連携成立ログがI18N経由で記録されること');
