...
| Trial🛡️消費→GameState | **PARTIAL / 経路実装済み** | Deployment Economy CommitではGameEngine/DefenseSystem経由により通常`currentDefense`へwrite-throughし、支払い失敗時は内部rollback。正規cost profile未確定のため通常ラン常時有効化は未完了。 |
