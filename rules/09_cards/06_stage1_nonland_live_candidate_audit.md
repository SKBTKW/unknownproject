...
現在のsource JSONと実行系を照合すると、**そのままlive化を試験できる候補は3枚**に絞られる。
...
表示は「指定した湿原1マス」だが、現実装はLegacy branch内で対象を自動探索する。
Target選択責務をBoard/Domain Actionへ移すまでDormant。
...
### `CMD_GRANARY`
表示は「全体の食料維持費 ×0.90」。
現effectは `granaryCount += 1` だが、現Maintenance経路はこの値を消費しない。
見た目だけの施設になってしまう。
